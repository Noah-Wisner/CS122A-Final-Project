...
static uint8_t hb_counter = 0;     // counts every DIST tick independently
...
bool readDHT(int16_t *temp_x10, uint16_t *hum_x10) {
    ...
    for (uint i = 0; i < 85; i++) {
        ...
        while (gpio_get(DATA) == last) {
            if ((time_us_32() - pulse_start) >= 255) break;
        }
        uint32_t count = time_us_32() - pulse_start;
        last = gpio_get(DATA);
        if (count >= 255) break;
    ...
    if ((j >= 40) && (data[4] == ((data[0] + data[1] + data[2] + data[3]) & 0xFF))) {
        ...
        if (data[2] & 0x80) t = -t;
        *temp_x10 = t;
        *hum_x10  = (uint16_t)((data[0] << 8) | data[1]);
        return true;
    }
...
// Owns all CAN TX — computes flags, sends on change; heartbeat on its own counter
int TickFct_DIST(int state) {
    ...
    // Send flags frame only when something changed
    if (flags != prev_flags) {
        can_send(CAN_ID_FLAGS, &flags, 1);
        prev_flags = flags;
    }

    // Heartbeat advances every tick regardless of flag changes
    hb_counter++;
