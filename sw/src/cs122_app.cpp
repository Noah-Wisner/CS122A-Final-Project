...
namespace ucr { namespace bcoe { namespace cs { namespace cs122 {
    ...
    void CS122_App::create_top_bar() 
    {
        ...
        system_state_label = create_label(top_bar, "SYSTEM: DISARMED", LV_ALIGN_LEFT_MID, 10, 0);
        alert_count_label = create_label(top_bar, "Alerts: 0", LV_ALIGN_RIGHT_MID, -10, 0);
    }
    ...
    void CS122_App::show_dashboard() 
    {
        ...
        // Alarm status
        create_label(content_area, "ALARM:", LV_ALIGN_TOP_RIGHT, -120, 10);
        alarm_status_label = create_label(content_area, "OFF", LV_ALIGN_TOP_RIGHT, -40, 10);

        // Zone panel
        lv_obj_t* zone_panel = lv_obj_create(content_area);
        lv_obj_set_size(zone_panel, 220, 160);
        lv_obj_set_pos(zone_panel, 0, 35);
        lv_obj_clear_flag(zone_panel, LV_OBJ_FLAG_SCROLLABLE);

        //static labels
        create_label(zone_panel, "ZONE", LV_ALIGN_TOP_MID, 0, 5);
        ...
        // Event log panel
        lv_obj_t* log_panel = lv_obj_create(content_area);
        lv_obj_set_size(log_panel, 220, 160);
        lv_obj_set_pos(log_panel, 220, 35);
        ...
        zone_panel_ptr = zone_panel;
        log_panel_ptr = log_panel;
    }

    void CS122_App::update_alarm_status(bool active) 
    {
        lv_label_set_text(alarm_status_label, active ? "ACTIVE" : "OFF");
    }

    void CS122_App::update_zone_status(bool online) 
    {
        lv_label_set_text(zone_status_label, online ? "ONLINE" : "OFFLINE");
    }

    void CS122_App::update_door_status(bool open) 
    {
        lv_label_set_text(door_status_label, open ? "OPEN" : "CLOSED");
    }

    void CS122_App::update_motion_status(bool detected) 
    {
        lv_label_set_text(motion_status_label, detected ? "DETECTED" : "CLEAR");
    }

    void CS122_App::update_environment_status(bool highTemp, bool lowTemp, bool highHum) 
    {
        if (highTemp) lv_label_set_text(environment_label, "HIGH TEMP");

        else if (lowTemp) lv_label_set_text(environment_label, "LOW TEMP");
        
        else if (highHum) lv_label_set_text(environment_label, "HIGH HUM");
        
        else lv_label_set_text(environment_label, "NORMAL");
    }

    void CS122_App::update_event_log(const char* events[5]) {
        for (int i = 0; i < 5; i++)
    ...
    void CS122_App::update_system_state(SystemState state) 
    {
        const char* text;
        switch (state) {
            case DISARMED:  text = "SYSTEM: DISARMED";  break;
            case ARMED:     text = "SYSTEM: ARMED";     break;
            case ALERT:     text = "SYSTEM: ALERT";     break;
            case FAULT:     text = "SYSTEM: FAULT";     break;
            default:        text = "SYSTEM: UNKNOWN";   break;
        }
        lv_label_set_text(system_state_label, text);
    }

    void CS122_App::update_alert_count(uint32_t count) 
    {
        char buffer[32];
        snprintf(buffer, sizeof(buffer), "Alerts: %lu", (unsigned long)count);
        lv_label_set_text(alert_count_label, buffer);
    }
    ...
    void CS122_App::refresh_dashboard(const ZoneStatus& zone, SystemState state, uint32_t alertCount) 
    {
        ...
        if (zone_panel_ptr) lv_obj_invalidate(zone_panel_ptr);
        if (log_panel_ptr) lv_obj_invalidate(log_panel_ptr);
    }
